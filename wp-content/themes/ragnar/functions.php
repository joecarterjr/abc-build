...
function ragnar_add_this_script_footer(){ 
	...
	$ragnar_script = '	
		"use strict"; 
		jQuery(document).ready(function($){	
			jQuery(".searchform #s").attr("value","'. esc_html__("Search and hit enter...","ragnar").'");	
...
return '<div class="ragnar-read-more"><a class="more-link" href="' . get_permalink() . '">'.esc_html__('Continue reading','ragnar').'</a></div>';
}
...
function ragnar_estimated_reading_time( $id) {
	...
	if($words < 100){
		$words = count(preg_split('/\s+/u', strip_tags( $post-> post_content ) , null, PREG_SPLIT_NO_EMPTY));
	}
...
?>
	<div class="block1">
	  <div class="block1_inner">
		<a href="<?php echo esc_url($ragnar_data['block1_link1']) ?>" title="<?php esc_attr_e('Image','ragnar'); ?>">
		...
		<a href="<?php echo esc_url($ragnar_data['block1_link2']) ?>" title="<?php esc_attr_e('Image','ragnar'); ?>" >							
		...
		<a href="<?php echo esc_url($ragnar_data['block1_link3']) ?>" title="<?php esc_attr_e('Image','ragnar'); ?>" >								
			<div class="block1_img">
...
function ragnar_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            ...
            'version'            => '5.4.6.3.1', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
